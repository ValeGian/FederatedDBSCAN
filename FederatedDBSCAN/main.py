...
if __name__ == '__main__':
    ...
    cells, labels = srv.compute_clusters(contribution_map)
    #plt.plot2Dcluster(cells, labels)
    #print(f'len: {len(points)}, points: {points}, shape: {points.shape}')
    #print(f'len: {len(labels)}, labels: {labels}')

    for i in range(M):
        points, labels = lcl.assign_points_to_cluster(i, cells, labels)
